...
if [ "${1}" = "modules" ]; then
  ...
  if [ "$TARGET_PLATFORM" = "apollolake" ]||[ "$TARGET_PLATFORM" = "geminilake" ]||[ "$TARGET_PLATFORM" = "epyc7002" ]; then
    tar xvfz /exts/all-modules/firmwarei915.tgz -C /lib/firmware/ >/dev/null 2>&1
  fi  
  ...
  if [ "$TARGET_PLATFORM" = "apollolake" ]||[ "$TARGET_PLATFORM" = "geminilake" ]||[ "$TARGET_PLATFORM" = "epyc7002" ]; then  
    tar xvfz /exts/all-modules/firmwarei915.tgz -C /tmpRoot/lib/firmware/ >/dev/null 2>&1
  fi  
  #if [ ${REVISION} = "#64570" ]; then
