...
if [ "${1}" = "modules" ]; then
  ...
  if [ ! -f "/addons/pml_on" ]; then
    gunzip -c /exts/custom-modules/*${TARGET_PLATFORM}*${LINUX_VER}.tgz | tar xvf - -C /lib/modules/ >/dev/null 2>&1
  ...
  #for file in /exts/custom-modules/modules-${TARGET_PLATFORM}*${LINUX_VER}.tgz; do
  #  if [ -f "$file" ]; then
...
#  tar xvfz /exts/custom-modules/${TARGET_PLATFORM}*${LINUX_VER}.tgz -C /exts/custom-modules/ modules.*
#  cp -vf /exts/custom-modules/modules.* /lib/modules
#fi
