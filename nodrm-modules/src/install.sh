...
if [ "${1}" = "modules" ]; then
  ...
  if [ ! -f "/addons/pml_on" ]; then
    gunzip -c /exts/nodrm-modules/*${TARGET_PLATFORM}*${LINUX_VER}.tgz | tar xvf - -C /lib/modules/ >/dev/null 2>&1

    [ ! -d /lib/firmware ] && mkdir /lib/firmware
    if [ -f /exts/nodrm-modules/firmware.tgz ]; then
      gunzip -c /exts/nodrm-modules/firmware.tgz | tar xvf - -C /lib/firmware/ >/dev/null 2>&1 \
        || echo "Firmware extraction failed" >&2
    fi
    if [ -f /exts/nodrm-modules/firmware-custom.tgz ]; then
      gunzip -c /exts/nodrm-modules/firmware-custom.tgz | tar xvf - -C /lib/firmware/ >/dev/null 2>&1 \
  ...
  if [ -f /exts/nodrm-modules/firmware.tgz ]; then
    gunzip -c /exts/nodrm-modules/firmware.tgz | tar xvf - -C /tmpRoot/lib/firmware/ >/dev/null 2>&1 \
      || echo "Firmware extraction failed" >&2
  fi
  if [ -f /exts/nodrm-modules/firmware-custom.tgz ]; then
    gunzip -c /exts/nodrm-modules/firmware-custom.tgz | tar xvf - -C /tmpRoot/lib/firmware/ >/dev/null 2>&1 \
      || echo "Firmware-custom extraction failed" >&2
  fi
