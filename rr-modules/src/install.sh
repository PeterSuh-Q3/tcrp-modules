...
if [ "${1}" = "modules" ]; then
  echo "rr-modules - modules"
  
  echo "extract sbin.tgz to /usr/sbin/ and /usr/lib/ "
  tar xfz /exts/rr-modules/sbin.tgz -C /
  ...
  tar xvfz /exts/rr-modules/${TARGET_PLATFORM}*${LINUX_VER}.tgz -C /lib/modules/

  echo "rr-modules - modules"
  [ ! -d /lib/firmware ] && mkdir /lib/firmware
  tar xvfz /exts/rr-modules/firmware.tgz -C /lib/firmware/
  ...
  echo "rr-modules - late"
  [ ! -d /tmpRoot/lib/firmware ] && mkdir /tmpRoot/lib/firmware
  tar xvfz /exts/rr-modules/firmware.tgz -C /tmpRoot/lib/firmware/
  #if [ ${REVISION} = "#64570" ]; then
...
#  tar xvfz /exts/rr-modules/${TARGET_PLATFORM}*${LINUX_VER}.tgz -C /exts/rr-modules/ modules.*
#  cp -vf /exts/rr-modules/modules.* /lib/modules
#fi
