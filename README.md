...
| `custom-modules/` | Independent custom module channel, primarily for Kernel 5.10.55 targets. These packages are produced separately from `all-modules` and may use their own kernel build context. They must not be assumed to be identical to the primary module pack. **Critical packaging rule: every 5.10.55 platform archive must include that platform's compatible RR `synobios.ko` and a `synobios.ko:` entry in `modules.dep`. If it is missing, `/dev/synobios` cannot be served even if its device node exists; DSM web login (including port 5000) can then trigger an immediate shutdown/reboot. For the current 7.3 and 7.4 5.10.55 releases, use the platform-matched 7.4 RR `synobios.ko`, and regenerate the release JSON SHA-256 after repacking.** |
...
| `custom-modules/` | 주로 Kernel 5.10.55 대상을 위한 독립 커스텀 모듈 공급 경로입니다. `all-modules`와 별도로 생성되며 자체 커널 빌드 환경을 사용할 수 있으므로 두 모듈팩을 동일한 것으로 간주하면 안 됩니다. **중요한 패키징 규칙: 5.10.55 플랫폼 아카이브에는 반드시 해당 플랫폼과 호환되는 RR의 `synobios.ko` 및 `modules.dep`의 `synobios.ko:` 항목을 포함해야 합니다. 이것이 누락되면 장치 노드가 있어도 `/dev/synobios`를 제공하지 못하며, DSM 웹 로그인(5000 포트 포함) 시 즉시 전원 종료 또는 재부팅이 발생할 수 있습니다. 현재 7.3·7.4의 5.10.55 릴리즈는 플랫폼에 맞는 RR 7.4 `synobios.ko`를 사용하고, 아카이브를 다시 만들었으면 반드시 릴리즈 JSON의 SHA-256도 갱신합니다.** |
